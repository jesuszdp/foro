...
<?php
    if(isset($data_asignados))
    {
        if($data_asignados['success'])
        {
            if(count($data_asignados['result']) > 0)
            {
?>
<!--  lista_revisados -->
<table class="table">
  ...
  <tbody>
    ...
    foreach ($data_asignados['result'] as $row)
    {
      ?>
      <tr>
...
{
  ?>
  <h3><?php echo $data_asignados['msg'];?></h3>
  <?php
}
?>
...
?>
<script>
$("#asignados").removeClass("active")
$("#sin_asignar").removeClass()
</script>
