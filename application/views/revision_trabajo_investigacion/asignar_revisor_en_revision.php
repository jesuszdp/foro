<div class="modal-header">
    <h3 class="modal-title" id="exampleModalLabel">Asignar revisor al folio: <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true">&times;</span>
  </button></h3>
</div>
<div class="modal-body"><div id="resultado_asignacion"></div>
    <ul>
        <?php echo "<li class='bullet-folio'>".$folio."</li>"; ?>
    </ul>
      ...
      <tbody>
        <?php foreach ($revisores as $key_r => $revisor) {
            ...
            echo '<tr>
                ...
                <td>'.$this->form_complete->create_element(array('id'=>'check_'.$id_usuario, 'type'=>'radio', 'value'=>$id_usuario, 'attributes'=>array('class'=>'check_asignar_usuario', 'name'=>'usuario_nuevo'))).'</td>
            </tr>';
        }
        ?>
      </tbody>
    ...
    <?php 
    echo $this->form_complete->create_element(array('id'=>'folio', 'type'=>'hidden', 'value'=>$folio_enc));
    echo $this->form_complete->create_element(array('id'=>'usuario_anterior', 'type'=>'hidden', 'value'=>$usuario_anterior));
    echo form_close(); ?>
</div>
...
$(document).ready(function () {
    $("#btn_asignar").on('click', function (e) {
        ...
        if(numberOfChecked==1){
            data_ajax(site_url + '/gestion_revision/asignar_revisor_en_revision_bd', "#asignar_revisor_form", "#resultado_asignacion", function () {
                //alert('Yea!');
