...
class Reportes_calidad_model extends MY_Model {
     ...
     * 
     */
    public function get_calidad_nacionales_institucion_umae($nacional = true) {
        $this->db->flush_cache();
        $this->db->reset_query();
        $select = array(
            "concat(uni.nombre, ' (', uni.clave_unidad, ')') nombre", "avg(d.promedio) promedio"
            , "count(distinct ti.folio) total_trabajos"
        );
        $this->db->select($select);
        $this->db->join("foro.dictamen d", "d.folio = ti.folio", "inner");
        $this->db->join("foro.autor au", "au.folio_investigacion = ti.folio and au.registro", "inner", false);
        $this->db->join("sistema.informacion_usuario iu", "iu.id_informacion_usuario = au.id_informacion_usuario", "inner");
        $this->db->join("sistema.historico_informacion_usuario hiu", "hiu.id_informacion_usuario = iu.id_informacion_usuario and hiu.actual", "inner");
        $this->db->join("catalogo.departamento dep", "dep.clave_departamental = hiu.clave_departamental", "inner");
        $this->db->join("catalogo.unidad uni", "uni.clave_unidad = dep.clave_unidad", "inner");
        if ($nacional) {
            $this->db->where("iu.clave_pais", "MX");
        } else {
            $this->db->where("iu.clave_pais <> 'MX'");
        ...
        $this->db->where("iu.es_imss", true);
        $this->db->where("uni.es_umae", true);
        $this->db->group_by("uni.nombre, uni.clave_unidad");
     ...
     * 
     */
    public function get_calidad_nacionales_externos($nacional) {
        $this->db->flush_cache();
        $this->db->reset_query();
        $select = array(
            "concat(uni.nombre, ' (', uni.clave_unidad, ')') nombre", "avg(d.promedio) promedio"
        ...
        $this->db->where("iu.es_imss", FALSE);
        $this->db->group_by("");
        $this->db->order_by("");
